...
export default function ADSyncPage() {
  ...
  const sync = async () => { setBusy(true); setMessage(""); try { const r = await fetch("/api/admin/ad-sync", { method: "POST" }); const data = await r.json(); setMessage(r.ok ? `همگام‌سازی انجام شد: ${data.result?.users ?? 0} کاربر و ${data.result?.groups ?? 0} گروه.` : data.error || "خطا در همگام‌سازی"); await load(); } catch { setMessage("ارتباط با سرویس همگام‌سازی برقرار نشد."); } finally { setBusy(false); } };
